...
async def ingest_products(ac_client: ArmorCodeClient, port_token: str, dry_run: bool = False):
    ...
    for product in products:
        try:
            ...
            upsert_entity("armorcodeProduct", entity, port_token, dry_run=dry_run)
...
async def ingest_subproducts(ac_client: ArmorCodeClient, port_token: str, dry_run: bool = False):
    ...
    for subproduct in subproducts:
        try:
            ...
            upsert_entity("armorcodeSubProduct", entity, port_token, dry_run=dry_run)
        except Exception as e:
...
async def main(dry_run: bool = False):
    ...
    async with ArmorCodeClient(api_key) as ac_client:
        # 4. Run Ingestion
        await ingest_products(ac_client, port_token, dry_run=dry_run)
        await ingest_subproducts(ac_client, port_token, dry_run=dry_run)
        await ingest_findings_and_vulnerabilities(ac_client, port_token, dry_run=dry_run)

    logging.info("Integration run completed successfully!")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Armorcode to Port Integration Script.")
    parser.add_argument("--dry-run", action="store_true", help="Run the script without making any changes to Port.")
